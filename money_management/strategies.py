...
def strategy_4_dd_max_historique(history, capital, ratio_threshold=0.7, low_risk=0.5):
    """
    ...
    return low_risk if ratio > ratio_threshold else 1.0


def strategy_5_scaling_lineaire_capital(history, capital, gain_step=10, increment=0.1):
    """
    ...
    return 1.0 + (steps * increment)


def strategy_6_scaling_geometrique(history, capital, growth_rate=1.1, step=10):
    """
    ...
    return 1.0 * (growth_rate ** steps)


def strategy_7_risk_reset(history, capital, plateau_step=5, reset_risk=1.5):
    """
    ...
    return reset_risk if trades_since_ath >= plateau_step else 1.0


def strategy_8_ath_distance(history, capital, ath_distance=10, boost_risk=1.2):
    """
    ...
    return boost_risk if distance < ath_distance else 1.0


def strategy_9_anti_martingale_inversee(history, capital, up_factor=1.2, down_factor=0.8):
    """
    ...
    return down_factor if was_win else up_factor


def strategy_10_pertes_consecutives(history, capital, loss_streak=3, reduced_risk=0.5):
    """
    ...
    return reduced_risk if consecutive_losses else 1.0


def strategy_11_gestion_grosses_pertes(history, capital, threshold_R=3, emergency_risk=0.3):
    """
    ...
    return emergency_risk if loss_R >= threshold_R else 1.0


def strategy_12_anti_martingale_classique(history, capital, up_factor=1.2, down_factor=0.8):
    """
    ...
    return up_factor if was_win else down_factor


def strategy_13_serie_gains(history, capital, gain_streak=3, boosted_risk=1.5):
    """
    ...
    return boosted_risk if consecutive_wins else 1.0


def strategy_14_heat_ramp(history, capital, ramp_factor=0.1, streak_limit=5):
    """
    ...
    return 1.0 + (consecutive_wins * ramp_factor)


def strategy_15_volatilite_interne(history, capital, window=10, vol_factor=0.5):
    """
    ...
    if volatility > 0.05:
        return max(0.25, 1.0 - vol_factor * (volatility - 0.05) * 10)
    
    return 1.0


def strategy_16_stress_index(history, capital, window=20, stress_factor=0.3):
    """
    ...
    if stress > 1.5:  # Stress élevé
        return max(0.25, 1.0 - stress_factor * (stress - 1.5))
    
    return 1.0


def strategy_17_surprise_trade(history, capital, gain_threshold=5, loss_threshold=3):
    """
    ...
    if last_trade['profit_loss'] > 0 and multiplier >= gain_threshold:
        return 1.3  # Boost après gros gain
    elif last_trade['profit_loss'] < 0 and multiplier >= loss_threshold:
        return 0.5  # Réduction après grosse perte
    
    return 1.0


def strategy_18_deviation_vs_esperance(history, capital, window=50, up_factor=1.2):
    """
...
def strategy_19_risk_corridor(history, capital, dd_threshold=10, streak_threshold=4):
    """
    ...
    if dd_signal and streak_signal:
        return 0.25  # Réduction drastique
    elif dd_signal or streak_signal:
        return 0.5  # Réduction modérée
    
    return 1.0


def strategy_20_modele_lineaire_3_signaux(history, capital, a=0.3, b=0.4, c=0.3):
    """
    ...
    # Risque final: 1.0 si signaux = 0, 0.25 si signaux = max
    return max(0.25, 1.0 - 0.75 * risk_reduction)
...
STRATEGIES = {
    ...
    'strategy_4': {
        ...
        'params': {'ratio_threshold': 0.7, 'low_risk': 0.5},
